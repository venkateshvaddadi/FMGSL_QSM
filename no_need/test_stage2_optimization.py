...
# -*- coding: utf-8 -*-
"""
Created on Sun Jul  5 13:40:00 2026
...
print('=' * 75)
print('LAUNCHING SCAN-SPECIFIC TEST-TIME OPTIMIZATION INFERENCE ENGINE')
print('=' * 75)
...
# =====================================================================
# 2. ADDITIONAL EXPERIMENTAL OBJECTIVES (Physics DC & Contrastive Modules)
# =====================================================================

class DataConsistencyLoss(nn.Module):
    def __init__(self, matrix_size=(176, 176, 160)):  # Matches full resolution testing size
        super().__init__()
...
class TestTimeContrastiveLoss(nn.Module):
    ...
    def forward(self, z_pred, z_7t_ref):
        ...
        # Pull the current prediction embedding directly toward the high-quality reference domain vector
...
class FrozenSwinUNETRTeacher(nn.Module):
    ...
    def forward(self, x):
        # Full volumes are downsampled temporarily inside the teacher pipeline to match its structural feature bounds
        x_scaled = F.interpolate(x, size=(64, 64, 64), mode='trilinear', align_corners=True)
        hidden_states_out = self.swin_unetr.swinViT(x_scaled)
        pooled = self.gap(hidden_states_out[4])
        return pooled.view(pooled.size(0), -1)

# =====================================================================
# 3. GLOBAL CONFIGURATION & CORE ENVIRONMENT RUNNER
# =====================================================================
...
# ⚙️ INFERENCE OPTIMIZATION PARAMETERS
TEST_TIME_ITERS = 25  # Number of fine-tuning steps performed per patient volume
TEST_TIME_LR    = 5e-5 # Fine-tuning learning rate (keeps changes subtle and highly stable)
...
# Base checkpoint file used as a warm start initialization[cite: 1, 2]
...
# Initialize frozen feature teacher modules
...
# Load a high-quality 7T target domain patch vector to establish our semantic pull direction[cite: 1, 2]
ref_7t_file = os.listdir(ref_7t_path)[0]
...
# =====================================================================
# 4. ACTIVE SCAN-SPECIFIC ADAPTATION LOOP[cite: 1, 2]
# =====================================================================
print(f"\n[*] Beginning scan-specific adaptation loop. Fine-tuning models per test scan...")
print("-" * 85)

for i in patients_list:
    print(f"\n>>>> 🔄 Loading and Configuring Fresh Weights Instance for Patient_{i}")
    
    # Reload original baseline weights for each subject to ensure a clean starting point[cite: 1, 2]
    dw = QSMnet().to(device)
    state_dict = torch.load(warm_start_checkpoint, map_location=device)
    if list(state_dict.keys())[0].startswith('module.'):
        state_dict = {k.replace('module.', ''): v for k, v in state_dict.items()}
    dw.load_state_dict(state_dict)
    dw.train()  # Must be set to train to allow local backpropagation updates[cite: 1, 2]
    
    # Configure an isolated test-time optimizer for this scan[cite: 1, 2]
    ...
    for j in range(1, 6):
        ...
        # --- Test-Time Adaptation Iteration Block ---
        for step in range(TEST_TIME_ITERS):
            ...
            # Extract current step semantic characteristics
            z_pred = teacher_net(sus_cal)
            
            # Compute balancing losses
            loss_dc  = criterion_dc(sus_cal, phs, msk)
            loss_sem = criterion_sem(z_pred, z_7t_target)
            
            # Physics-constrained loss combination[cite: 1, 2]
            total_step_loss = loss_dc + (0.01 * loss_sem)
            
            total_step_loss.backward()
            test_time_optimizer.step()

        # Final evaluation processing pass for this orientation
        dw.eval()
        with torch.no_grad():
        ...
        # Calculate metrics using your precise verification equations
        ...
        print(f"   ↳ Orientation {j} Optim Complete | SSIM: {ssim_val:.4f} | PSNR: {psnr_val:.2f} dB | RMSE: {rmse_val:.4f}%")

        results.append({
            'patient': i, 'orientation': j, 'ssim': ssim_val, 'psnr': psnr_val, 'rmse': rmse_val, 'hfen': hfen_val
        })
        dw.train() # Reset back to train layout for the next orientation loop
...
plt.close()
print(f"✅ Active scan adaptation process successfully terminated. Outputs stored in {outdir}")